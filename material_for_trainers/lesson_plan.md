...
- [Demo] Failed push to a repository to explain why forking is necessary in some cases [5 min]
    ...
    - The lead instructor wishes to contribute a new recipe, so they open a new issue on the co-instructor's repository describing the change they wish to add, then clone the repository to their local machine, create a new branch, add the new recipe, commit the change, and try to push the changes to the original repository on GitHub. However, since the lead instructor does not have write access to the original repository, the push is rejected by GitHub.
