...
- [Demo] Forking workflow demo [15 min]
    ...
    - The lead instructor demonstrates the forking workflow by forking the `forking-workflow-demo` repository to their own GitHub account, cloning the forked repository to their local machine, creating a new branch, adding a new recipe, committing and pushing the changes to their fork on GitHub, and then creating a pull request from their branch in their fork to the main branch of the upstream `forking-workflow-demo` repository. 
